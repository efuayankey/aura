...
export default function SchedulePage() {
  ...
  const handleTaskCompletion = (scheduleItemId: string, taskId: string) => {
    ...
    const newCompletedTasks = taskId && !completedTasks.includes(taskId) ? [...completedTasks, taskId] : completedTasks
    ...
    return (
      <main className="min-h-screen flex items-center justify-center p-8">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-4">🤖 AI is analyzing your day...</h2>
          ...
          <p className="text-gray-500 mt-6 text-sm">Using OpenAI GPT-4 for intelligent scheduling</p>
        </div>
      </main>
    )
  }
    ...
    return (
      <main className="min-h-screen p-4">
        <div className="text-center mb-8 animate-fade-in">
          <h1 className="text-4xl md:text-6xl font-bold bg-gradient-to-r from-serenity-600 via-mindful-600 to-balance-600 bg-clip-text text-transparent mb-4">
            AURA
          </h1>
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">Your AI Balance Agent - Where wellness meets productivity</p>
        </div>
    ...
    <main className="min-h-screen p-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">Crafted by AI for optimal balance and productivity</p>
          <button onClick={() => setCurrentStep('input')} className="mt-4 px-6 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg transition-all">
            ← Adjust Settings
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2">
            <div className="glass rounded-3xl p-6">
              <h3 className="text-xl font-semibold mb-4 text-gray-800">Today's Schedule</h3>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <button
                          onClick={() => (isCompleted ? handleTaskUndo(item.id, item.taskId) : handleTaskCompletion(item.id, item.taskId))}
                          className={`mt-1 p-1 rounded-full transition-all ${isCompleted ? 'text-green-600 hover:text-green-700' : 'text-gray-400 hover:text-green-500'}`}
                        >
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className={`font-medium transition-all ${isCompleted ? 'text-gray-500 line-through' : 'text-gray-800'}`}>{item.title}</h4>
                              <p className={`text-sm mt-1 transition-all ${isCompleted ? 'text-gray-400' : 'text-gray-600'}`}>{item.description}</p>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className={`text-sm font-medium transition-all ${isCompleted ? 'text-gray-400' : 'text-gray-500'}`}>
                                {item.startTime} - {item.endTime}
                              </span>
                              {isWorkItem && (
                                <div className={`p-1 rounded ${isCompleted ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="glass rounded-3xl p-6">
              <h3 className="text-lg font-semibold mb-4 text-gray-800">Today's Stats</h3>
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-600">Completed</span>
                  <span className="font-medium text-green-600">{completedScheduleItems.length}/{schedule.length}</span>
                </div>
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${schedule.length > 0 ? (completedScheduleItems.length / schedule.length) * 100 : 0}%` }}
                      />
                    </div>
                    <span className="text-sm font-medium text-green-600">{Math.round(schedule.length > 0 ? (completedScheduleItems.length / schedule.length) * 100 : 0)}%</span>
                  </div>
                </div>
                <div className="flex justify-between">
